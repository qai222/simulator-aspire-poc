...
def get_layout_figure(lab: Lab) -> go.Figure:
    ...
    for k, v in lab.dict_object.items():
        if isinstance(v, (TecanSlot, TecanWashBay, TecanHotel, TecanLiquidTank)):
            x0, y0 = v.layout.layout_position
            x1 = x0 + v.layout.layout_x
            y1 = y0 + v.layout.layout_y

            if isinstance(v, TecanSlot) and v.slot_content['SLOT'] is not None:
                fillcolor = "gray"
            else:
                fillcolor = None
            ...
            if z1_arm.position_on_top_of == v.identifier and z2_arm.position_on_top_of == v.identifier:
                bgcolor = "purple"
            elif z1_arm.position_on_top_of == v.identifier and z2_arm.position_on_top_of != v.identifier:
                bgcolor = "red"
            elif z1_arm.position_on_top_of != v.identifier and z2_arm.position_on_top_of == v.identifier:
                bgcolor = "blue"
            else:
...
with open("sim_tecan_dummy.pkl", "rb") as f:
    sim_logs = pickle.load(f)

app = Dash(__name__, external_stylesheets=[dbc.themes.BOOTSTRAP], title="TECAN SIMULATOR")

card_layout = dbc.Card(
    [
        dbc.CardHeader("TECAN Layout"),
        dbc.CardBody(
            dcc.Graph(
                style={'height': '600px'},
...
card_tracker1 = dbc.Col(dbc.Card(
    [
        ...
        dbc.CardBody(
            [
                dcc.Dropdown(id="tracker-1-select", options=get_tracker_options(sim_logs), value="ARM-2"),
    ...
    dbc.Container(
        [
            html.A(
                ...
                dbc.NavbarBrand("Coley Lab TECAN Simulator", className="ms-2"),
